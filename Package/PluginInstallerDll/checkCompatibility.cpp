...
void GetSystemInfo()
{
	...
	if (GetModuleFileName(openclLib, buffer, MAX_PATH))
	{
		...
		LogSystem("   GetSystemInfo: OpenCL.dll path - %s", ws2s(s).c_str());
	}
	else
	{
...
{
	//this is the list of incompatible devices known by the Radeon ProRender team.
	...
	{
		if ( deviceName == (*it)) //incompatible device found
			return true;
	}
...
bool checkCompatibility_hardware(std::wstring &retMessage)
{
	...
	if (displaWarningMessage_openCLLoadFailed)
	{
		...
		LogSystem("!Warning: %s", ws2s(retMessage).c_str());
		return false;
	}
...
// 'rawDriver' is the input
//example of input :
//"10.18.13.5456" ---> return publicVersionOut=35456
//"9.18.13.697"   ---> return publicVersionOut=30697
...
bool checkCompatibility_driver(std::wstring &retMessage)
{
	...
	{
		retMessage = L"Your graphics driver is likely to be incompatible with Radeon ProRender.\r\n";

		if (hardwareIsAMD)
		{
			wchar_t buffer[MAX_PATH];
			wsprintf(buffer, L"%d.%d", Supported_AMD_driverMajor, Supported_AMD_driverMinor);
			retMessage += L"For AMD devices, " + std::wstring(buffer) + L" or higher is required.";
		}
		else if (hardwareIsNV)
		{
			retMessage += L"For NVIDIA devices, " + Supported_NVIDIA_driver_string + L" or higher is required.";
		}
